...
void syscall_signal(trapframe_t *_, unsigned long pid);
...
void syscall_fork(trapframe_t *regs) {
    ...
    //     return;
    // }
    child->regs.sp_el0 = kmalloc(STACK_SIZE) + STACK_SIZE; /* allocate user stack */
    // uart_send_string("[ thread ");
    // uart_send_int(current->tid);
    ...
    memcopy(child->regs.sp_el0 - STACK_SIZE, current->regs.sp_el0 - STACK_SIZE, STACK_SIZE);
