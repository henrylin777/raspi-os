...
void buddy_init() {
    ...
    buddy_reserve_memory(buddy_allocator, 0x08000000, 0x08030000);  /* initramfs */
...
#ifdef CONFIG_MM_DEBUG
        uart_send_string("buddy system: insert page at ");
...
#ifdef CONFIG_MM_DEBUG
    uart_send_string("buddy system: allocate out a memory block of order ");
    uart_send_int(order);
...
void *buddy_alloc_page(unsigned long order)
{
    ...
    for (unsigned long cur_order = order; cur_order <= MAX_ORDER; ++cur_order) {
        ...
        if (area->n_free == 0) {
#ifdef CONFIG_MM_DEBUG
            uart_send_string("buddy system: order ");
...
#ifdef CONFIG_MM_DEBUG
        uart_send_string("buddy system: got available page of order ");
        uart_send_int(cur_order);
        DEBUG(" at ", (unsigned long) page, 0);
        if (cur_order != order) {
