...
#define slub_min_objects 256
#define slub_max_order 64
...
void slub_init_object(struct kmem_cache *cache, struct page *page, unsigned long objects) {
    ...
    DEBUG("slub allocator: init object at address: ", (unsigned long) start, 0);
...
void *slub_malloc(unsigned long size) {
    ...
    if (!cache) {
        uart_send_string("cannot malloc for size ");
        uart_send_int(size);
